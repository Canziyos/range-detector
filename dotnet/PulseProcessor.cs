...
{
    private const int SENSOR_OFFSET_US = 920;
    private const int THRESHOLD_MM     = 300;     // >30 cm ⇒ far
    private const int GRACE_MS         = 500;     // far for 0.5 s
    private const int RETRY_MS         = 5000;    // after failed OFF

    private static int Mm(int us) => Math.Max(0, us - SENSOR_OFFSET_US);
    ...
    {
        if (!line.StartsWith("pulse:", StringComparison.OrdinalIgnoreCase)) return Task.CompletedTask;
        if (!int.TryParse(line.AsSpan(6).Trim(), out int us))               return Task.CompletedTask;

        int mm = Mm(us);
        Console.WriteLine($"distance ≈ {mm} mm  (raw {us} µs)");
    ...
    public async Task TickAsync()
    {
        ...
        bool needOff =
            (now - _lastFarEnough).TotalMilliseconds >= GRACE_MS &&
            !_offSent && now >= _nextRetry;
